...
function createInitialForm(categoryId = SOS_CATEGORIES[0]?.id || "") {
  const category = getSosCategoryMeta(categoryId);
  const firstGroup = getSosGroupOptions(category.id)[0];
  const firstTag = getSosTagOptions(category.id, firstGroup?.id)[0];
...
export default function SosCenter() {
  ...
  const selectedCategory = useMemo(
    () => getSosCategoryMeta(form.categoryId),
    [form.categoryId],
  );
  const groupOptions = useMemo(
    () => getSosGroupOptions(form.categoryId),
    [form.categoryId],
  );
  const selectedGroup = useMemo(
    () => getSosGroupMeta(form.categoryId, form.groupId),
    [form.categoryId, form.groupId],
  );
  const tagOptions = useMemo(
    () => getSosTagOptions(form.categoryId, form.groupId),
    [form.categoryId, form.groupId],
  );
  const selectedTag = useMemo(
    () => getSosTagMeta(form.categoryId, form.groupId, form.tagId),
    [form.categoryId, form.groupId, form.tagId],
  );
  const loading = !Object.values(ready).every(Boolean);

  useEffect(() => {
    if (!groupOptions.some((group) => group.id === form.groupId)) {
      const firstGroup = groupOptions[0];
      const firstTag = getSosTagOptions(form.categoryId, firstGroup?.id)[0];
  ...
  }, [form.categoryId, form.groupId, groupOptions, selectedCategory.sensitive]);
  ...
  const categorySummary = useMemo(
    () => buildSosCategorySummary(publicTickets),
    [publicTickets],
  );
  const tagSummary = useMemo(
    () => buildSosTagSummary(publicTickets).slice(0, 8),
    [publicTickets],
  );

  const handleChange = (key, value) => {
    if (key === "categoryId") {
      const nextCategory = getSosCategoryMeta(value);
      const nextGroup = getSosGroupOptions(value)[0];
      const nextTag = getSosTagOptions(value, nextGroup?.id)[0];
      ...
      setForm((previous) => ({
        ...
        tagId: nextTag?.id || "",
        anonymous: nextCategory.sensitive ? true : previous.anonymous,
      }));
      return;
    }

    if (key === "groupId") {
      const nextTag = getSosTagOptions(form.categoryId, value)[0];
      setForm((previous) => ({
        ...previous,
        groupId: value,
        tagId: nextTag?.id || "",
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const groupMeta = getSosGroupMeta(form.categoryId, form.groupId);
      const tagMeta = getSosTagMeta(form.categoryId, form.groupId, form.tagId);
      ...
      await addDoc(collection(db, "users", currentUser.uid, "sosTickets"), {
        ...
        groupShortLabel: groupMeta.shortLabel || groupMeta.label,
        tagId: tagMeta.id,
        tagLabel: tagMeta.label,
        title: form.title.trim(),
        details: form.details.trim(),
        publicSummary: visibilityMeta.isSensitive
          ? ""
          : form.details.trim().slice(0, 280),
      ...
      const resetForm = createInitialForm(form.categoryId);
      setForm({
        ...resetForm,
        anonymous: selectedCategory.sensitive ? true : resetForm.anonymous,
      });
      setFeedback(
    ...
    <div className="page-wrap space-y-6">
      <section className="dark-panel p-6 sm:p-8">
        ...
        <p className="mt-4 max-w-3xl text-base leading-8 text-slate-300">
          ส่งเรื่องตามระดับความเร่งด่วน จัดหมวดหมู่แบบหมวดหลัก-หมวดย่อย-แท็ก
          รอการอนุมัติจาก Admin และติดตามผลแบบเรียลไทม์ได้จากหน้านี้
        </p>
        <div className="mt-8 grid gap-4 sm:grid-cols-3">
          ...
          <StatCard
            ...
            detail="Admin ต้องอนุมัติก่อนขึ้นบอร์ดสรุป"
          />
          <StatCard
            label="เรื่องที่ยังเปิดอยู่"
            value={myStats.active}
            detail={`${myStats.approved} เรื่องผ่านการอนุมัติแล้ว`}
          />
        </div>
      </section>
      ...
      <section className="grid gap-6 xl:grid-cols-[1.02fr_0.98fr]">
        <form onSubmit={handleSubmit} className="surface-panel p-6 sm:p-8">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <h3 className="mt-1 font-display text-3xl font-semibold tracking-[-0.05em] text-slate-950">
                ระบุปัญหาให้ชัดเพื่อให้ทีมช่วยได้เร็วขึ้น
              </h3>
            </div>
          </div>
            ...
            {SOS_LEVELS.map((level) => (
              <button
                ...
                className={`rounded-[22px] border px-4 py-4 text-left transition ${level.cardClass} ${
                  form.level === level.value ? "ring-2 ring-slate-950/10" : "opacity-85"
                }`}
              >
          ...
          <div className="mt-6 space-y-5">
            <div>
              <label className="field-label" htmlFor="sos-category">
                หมวดหมู่หลัก
              </label>
              <select
                id="sos-category"
            ...
            <div>
              <label className="field-label" htmlFor="sos-group">
                หมวดย่อย
              </label>
              <select
                id="sos-group"
              ...
              <p className="mt-3 text-sm leading-7 text-slate-500">
                ระบบจะใช้หมวดย่อยนี้ช่วยให้ Admin สรุปปัญหาได้เร็วขึ้น
              </p>
            </div>
            ...
            <div>
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {tagOptions.map((tag) => (
                  <button
                    key={tag.id}
                    type="button"
                    onClick={() => handleChange("tagId", tag.id)}
              ...
              <div className="mt-3 rounded-[20px] border border-slate-200 bg-slate-50 px-4 py-4 text-sm leading-7 text-slate-600">
                <span className="font-semibold text-slate-900">เลือกอยู่ตอนนี้:</span>{" "}
                {selectedCategory.shortLabel} / {selectedGroup.shortLabel || selectedGroup.label} /{" "}
                {selectedTag?.label || "-"}
              </div>
            </div>
            ...
            <div className="rounded-[22px] border border-slate-200 bg-slate-50 p-4 text-sm leading-7 text-slate-600">
              {selectedCategory.sensitive
                ? "หมวดนี้เป็นข้อมูลลับ ระบบจะตั้งค่าเป็น Anonymous และ Private ให้อัตโนมัติ"
                : "หากต้องการซ่อนชื่อบนบอร์ดสรุป สามารถเปิดโหมด Anonymous ได้"}
              {!selectedCategory.sensitive && (
          ...
          {feedback && (
            <div className="mt-6 rounded-[22px] border border-slate-200 bg-slate-50 px-4 py-4 text-sm leading-7 text-slate-700">
              {feedback}
            </div>
          )}
        ...
        <section className="space-y-4">
          <PanelHeader
            ...
            eyebrow="เรื่องของฉัน"
          />
          {loading ? (
            <div className="surface-panel flex min-h-[220px] items-center justify-center p-6">
              <Loader2 size={28} className="animate-spin text-slate-400" />
            </div>
          ) : tickets.length === 0 ? (
            <div className="surface-panel p-8 text-center text-sm leading-7 text-slate-500">
              เมื่อส่งเรื่องแล้ว ระบบจะบันทึกเรื่องของคุณไว้ที่นี่และอัปเดตสถานะให้อัตโนมัติ
            </div>
          ) : (
            tickets.map((ticket) => {
              ...
              return (
                <article key={ticket.path} className="surface-panel p-6">
                  <div className="flex flex-wrap gap-2">
                    <Chip className={levelMeta.badgeClass}>
                    ...
                    <Chip className="border-slate-200 bg-white text-slate-700">
                      {ticket.categoryShortLabel || ticket.categoryLabel}
                    </Chip>
                    {ticket.groupShortLabel || ticket.groupLabel ? (
                      <Chip className="border-slate-200 bg-white text-slate-700">
                        {ticket.groupShortLabel || ticket.groupLabel}
                      </Chip>
                    ) : null}
                    ...
                    {ticket.isSensitive && (
                      <Chip className="border-slate-900 bg-slate-950 text-white">ลับ</Chip>
                    )}
                    {ticket.anonymous && (
                      <Chip className="border-slate-200 bg-slate-100 text-slate-700">
                        Anonymous
                      </Chip>
                    )}
                  </div>
                  ...
                  <div className="mt-4 grid gap-3 sm:grid-cols-2">
                    ...
                    <InfoCard
                      label="ติดต่อกลับ"
                      value={ticket.preferredContact || "ยังไม่ได้ระบุ"}
                    />
                  </div>
                </article>
              );
          ...
          <PanelHeader
            icon={<Tags size={22} />}
            title="บอร์ดสรุป SOS ที่อนุมัติแล้ว"
            eyebrow="Approved Board"
          />
          <div className="surface-panel space-y-4 p-6">
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {categorySummary.length > 0 ? (
                  categorySummary.map((item) => (
                    <Chip key={item.id} className="border-slate-200 bg-white text-slate-700">
                      {item.icon} {item.shortLabel} {item.count}
                    </Chip>
                  ))
                ) : (
                  <span className="text-sm text-slate-500">
                    ยังไม่มีเรื่องที่ผ่านการอนุมัติ
                  </span>
                )}
              </div>
            </div>
            ...
            <div>
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {tagSummary.length > 0 ? (
                  tagSummary.map((item) => (
                    <Chip key={item.key} className="border-slate-200 bg-white text-slate-700">
                      {item.categoryLabel} / {item.groupLabel}: {item.label} ({item.count})
                    </Chip>
                  ))
                ) : (
                  <span className="text-sm text-slate-500">ยังไม่มีแท็กสรุป</span>
                )}
              </div>
            </div>
            ...
            <div className="space-y-3">
              {publicTickets.length === 0 ? (
                <div className="rounded-[22px] border border-dashed border-slate-300 bg-white px-4 py-5 text-sm leading-7 text-slate-500">
                  เรื่องในหมวดลับจะไม่ถูกแสดงบนบอร์ดนี้ และทุกเรื่องต้องผ่านการอนุมัติจาก
                  Admin ก่อนเสมอ
                </div>
              ) : (
                publicTickets.map((ticket) => (
                  <article
                    key={`${ticket.userId}-${ticket.title}`}
                  ...
                  >
                    ...
                    <p className="mt-2 text-sm leading-7 text-slate-600">
                      {ticket.publicSummary || ticket.details}
                    </p>
                    <div className="mt-3 text-xs text-slate-500">
                      โดย {ticket.anonymous ? "ไม่เปิดเผยชื่อ" : ticket.userName || "ผู้ใช้งาน"} ·{" "}
                      {formatLastSeenLabel(ticket.updatedAt || ticket.createdAt)}
                    </div>
                  </article>
                ))
              )}
